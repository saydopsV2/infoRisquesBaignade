...
const Table: React.FC<TableProps> = ({ indices }) => {
  ...
  return (
    <div className="w-full overflow-x-auto bg-slate-100 text-black">
      <table className="w-full border-collapse bg-slate-100 text-black">
        <thead>
          <tr className="bg-blue-500">
            <th className="p-2 text-left" colSpan={25}>
        ...
        <tbody>
          <tr className="border-b">
            <td className="p-2 font-bold border-r bg-gray-200">Heures</td>
            {hours.map((hour, index) => (
              <td key={`hour-${index}`} className="p-2 text-center border-r">
                {hour.getHours()}:00
              </td>
            ))}
          </tr>
          <tr className="bg-blue-100">
            <td className="p-2 font-bold border-r bg-gray-200">Indices</td>
            {safeIndices.slice(0, 24).map((indice, index) => {
              ...
              return (
                <td key={`indice-${index}`} className={`p-2 text-center border-r ${bgColorClass}`}>
                  {indice}
                </td>
              );
            })}
      ...
      <div className="mt-4 p-2 bg-slate-100 text-black">
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center">
            ...
            <span className="ml-2">0 - Sécurité optimale</span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-6 bg-green-400"></div>
            <span className="ml-2">1 - Faible risque</span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-6 bg-orange-100"></div>
            <span className="ml-2">2 - Risque modéré</span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-6 bg-orange-500"></div>
            <span className="ml-2">3 - Risque élevé</span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-6 bg-red-600"></div>
            <span className="ml-2">4 - Danger important</span>
          </div>
        </div>
      </div>
    </div>
